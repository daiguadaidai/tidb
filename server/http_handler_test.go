...
import (
	...
	"github.com/daiguadaidai/tidb/store/helper"
...
func (ts *HTTPHandlerTestSuite) TestRegionsFromMeta(c *C) {
	...
	// test no panic
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/server/errGetRegionByIDEmpty", `return(true)`), IsNil)
	resp1, err := http.Get("http://127.0.0.1:10090/regions/meta")
	c.Assert(err, IsNil)
	defer resp1.Body.Close()
	c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/server/errGetRegionByIDEmpty"), IsNil)
}
