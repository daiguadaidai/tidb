...
func (s *testTableCodecSuite) TestDecodeBadDecical(c *C) {
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/util/codec/errorInDecodeDecimal", `return(true)`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/util/codec/errorInDecodeDecimal"), IsNil)
	}()
