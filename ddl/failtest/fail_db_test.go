...
func (s *testFailDBSuite) TestHalfwayCancelOperations(c *C) {
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/ddl/truncateTableErr", `return(true)`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/ddl/truncateTableErr"), IsNil)
	}()
	...
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/ddl/renameTableErr", `return(true)`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/ddl/renameTableErr"), IsNil)
	}()
...
func (s *testFailDBSuite) TestInitializeOffsetAndState(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/ddl/uninitializedOffsetAndState", `return(true)`), IsNil)
	tk.MustExec("ALTER TABLE t MODIFY COLUMN b int FIRST;")
	c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/ddl/uninitializedOffsetAndState"), IsNil)
}

func (s *testFailDBSuite) TestUpdateHandleFailed(c *C) {
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/ddl/errorUpdateReorgHandle", `1*return`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/ddl/errorUpdateReorgHandle"), IsNil)
	}()
	tk := testkit.NewTestKit(c, s.store)
...
func (s *testFailDBSuite) TestAddIndexFailed(c *C) {
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/ddl/mockAddIndexErr", `1*return`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/ddl/mockAddIndexErr"), IsNil)
	}()
	tk := testkit.NewTestKit(c, s.store)
...
func (s *testFailDBSuite) TestFailSchemaSyncer(c *C) {
	...
	// make reload failed.
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/domain/ErrorMockReloadFailed", `return(true)`), IsNil)
	...
	c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/domain/ErrorMockReloadFailed"), IsNil)
...
func (s *testFailDBSuite) TestGenGlobalIDFail(c *C) {
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/ddl/mockGenGlobalIDFail"), IsNil)
	}()
	tk := testkit.NewTestKit(c, s.store)
	...
	for idx, test := range testcases {
		if test.mockErr {
			c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/ddl/mockGenGlobalIDFail", `return(true)`), IsNil)
			_, err := tk.Exec(test.sql)
			c.Assert(err, NotNil, Commentf("the %dth test case '%s' fail", idx, test.sql))
		} else {
			c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/ddl/mockGenGlobalIDFail", `return(false)`), IsNil)
...
func (s *testFailDBSuite) TestAddIndexWorkerNum(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/ddl/checkIndexWorkerNum", `return(true)`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/ddl/checkIndexWorkerNum"), IsNil)
	}()
...
func (s *testFailDBSuite) TestRunDDLJobPanic(c *C) {
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/ddl/mockPanicInRunDDLJob"), IsNil)
	}()
	tk := testkit.NewTestKit(c, s.store)
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t")
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/ddl/mockPanicInRunDDLJob", `1*panic("panic test")`), IsNil)
