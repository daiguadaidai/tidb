...
import (
	...
	"github.com/daiguadaidai/tidb/statistics/handle"
...
func (s *seqTestSuite) TestEarlyClose(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/store/tikv/handleTaskOnceError", `return(true)`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/store/tikv/handleTaskOnceError"), IsNil)
	}()
...
func (s *seqTestSuite) TestParallelHashAggClose(c *C) {
	...
	// Goroutine should not leak when error happen.
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/executor/parallelHashAggError", `return(true)`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/executor/parallelHashAggError"), IsNil)
	}()
	ctx := context.Background()
...
func (s *seqTestSuite) TestUnparallelHashAggClose(c *C) {
	...
	// Goroutine should not leak when error happen.
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/executor/unparallelHashAggError", `return(true)`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/executor/unparallelHashAggError"), IsNil)
	}()
	ctx := context.Background()
