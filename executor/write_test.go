...
func (s *testSuite4) TestAutoIDInRetry(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/session/mockCommitRetryForAutoID", `return(true)`), IsNil)
	tk.MustExec("commit")
	c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/session/mockCommitRetryForAutoID"), IsNil)
