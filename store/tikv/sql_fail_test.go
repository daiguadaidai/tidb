...
func (s *testSQLSuite) TestFailBusyServerCop(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/store/mockstore/mocktikv/rpcServerBusy", `return(true)`), IsNil)
	go func() {
		defer wg.Done()
		time.Sleep(time.Millisecond * 100)
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/store/mockstore/mocktikv/rpcServerBusy"), IsNil)
	}()
