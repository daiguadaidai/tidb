...
func (s *testSchemaSuite) TestLoadSchemaFailed(c *C) {
	...
	failpoint.Disable("github.com/daiguadaidai/tidb/domain/ErrorMockReloadFailed")
	time.Sleep(lease * 2)
...
func (s *testSessionSuite) TestTxnRetryErrMsg(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/store/tikv/ErrMockRetryableOnly", `return(true)`), IsNil)
	_, err := tk1.Se.Execute(context.Background(), "commit")
	c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/store/tikv/ErrMockRetryableOnly"), IsNil)
	c.Assert(err, NotNil)
