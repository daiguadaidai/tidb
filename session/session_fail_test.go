...
func (s *testSessionSuite) TestFailStatementCommit(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/session/mockStmtCommitError", `return(true)`), IsNil)
	_, err := tk.Exec("insert into t values (2),(3),(4),(5)")
	c.Assert(err, NotNil)

	c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/session/mockStmtCommitError"), IsNil)
...
func (s *testSessionSuite) TestFailStatementCommitInRetry(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/session/mockCommitError8942", `return(true)`), IsNil)
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/session/mockStmtCommitError", `return(true)`), IsNil)
	_, err := tk.Exec("commit")
	c.Assert(err, NotNil)
	c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/session/mockCommitError8942"), IsNil)
	c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/session/mockStmtCommitError"), IsNil)
...
func (s *testSessionSuite) TestGetTSFailDirtyStateInretry(c *C) {
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/session/mockCommitError"), IsNil)
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/store/tikv/mockGetTSErrorInRetry"), IsNil)
	}()

	tk := testkit.NewTestKitWithInit(c, s.store)
	tk.MustExec("create table t (id int)")

	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/session/mockCommitError", `return(true)`), IsNil)
	...
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/store/tikv/mockGetTSErrorInRetry",
...
func (s *testSessionSuite) TestRetryPreparedSleep(c *C) {
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/store/tmpMaxTxnTime"), IsNil)
	}()
	tk := testkit.NewTestKitWithInit(c, s.store)
	tk.MustExec("create table t (c1 int)")
	tk.MustExec("insert t values (11)")

	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/store/tmpMaxTxnTime", `return(2)->return(0)`), IsNil)
	tk.MustExec("begin")
