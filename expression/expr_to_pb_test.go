...
func (s *testEvaluatorSuite) TestPushDownSwitcher(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/expression/PushDownTestSwitcher", `return("all")`), IsNil)
	defer func() {
		c.Assert(failpoint.Disable("github.com/daiguadaidai/tidb/expression/PushDownTestSwitcher"), IsNil)
	}()
	...
	// All disabled
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/expression/PushDownTestSwitcher", `return("")`), IsNil)
	pbExprs = ExpressionsToPBList(sc, funcs, client)
	c.Assert(len(pbExprs), Equals, len(cases))
	for i, pbExpr := range pbExprs {
	...
	c.Assert(failpoint.Enable("github.com/daiguadaidai/tidb/expression/PushDownTestSwitcher", fpexpr), IsNil)
	pbExprs = ExpressionsToPBList(sc, funcs, client)
	c.Assert(len(pbExprs), Equals, len(cases))
	for i, pbExpr := range pbExprs {
		if !cases[i].enable {
